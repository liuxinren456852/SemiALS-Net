...
#             label_ohot_pl = tf.placeholder(dtype=tf.int64, shape=[BATCH_SIZE, NUM_POINT, NUM_CLASS], name='PartGT_ohot')  # B*N*9
#             Mask_ph = tf.placeholder(dtype=tf.bool, shape=[BATCH_SIZE, NUM_POINT], name='Mask')  # B*N

            logging.info("Loading model")
            if 'pointconv' in FLAGS.model:
                pred, end_points, seg_pred = MODEL.get_model(pointclouds_pl, is_training_pl, NUM_CLASSES, sigma=FLAGS.sigma, radius=FLAGS.radius, code=FLAGS.codeword)
            else:
...
#         model_file=tf.train.latest_checkpoint(MODEL_PATH)
        ...
        LPSolver = PLP.LabelPropagation_TF(alpha=1e0, beta=1e0, K=20)
        TFComp = {}
        ...
        while True:
            ...
            for k in range(len(batch_list)):
                ...
                pred_val, pred_prob = sess.run([ops['pred'], prob], feed_dict=feed_dict)
#                 pdb.set_trace()
#                 pred_labels = np.argmax(pred_val, 2)  # BxN
                
                ## Apply Label Propagation
                prob_LP_list = []
                for i in range(len(pred_prob)):
                    Lmat = TFComp['Lmat'].Eval(sess, aug_data[np.newaxis, i, :, 0:3], aug_data[np.newaxis, i, :, 3:4]) #[B,N,N] Laplacin matrix
                    _, Z_prob_LP, w = LPSolver.SolveLabelProp(sess, Lmat[0], pred_prob[i])
                    prob_LP_list.append(Z_prob_LP)
                prob_LP_list = np.stack(prob_LP_list, 0)
                pred_labels = np.argmax(prob_LP_list, 2)  # BxN
