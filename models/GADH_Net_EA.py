...
def get_model(point_cloud, is_training, num_class, bn_decay=None):
    ...
    l0_points = tf.slice(point_cloud, [0, 0, 3], [-1, -1, point_cloud.shape[-1]-3])
...
def get_eql_loss(pred, label, smpw):
    """
    :param pred: BxNxC
    :param label: BxN
    :param smpw: BxN
    ...
    eql_loss = EQL_loss(lamb=0.1, freq_info=None)
    # Deep supervision
    loss0 = eql_loss(labels=label, logits=pred[0], weights=smpw)
    loss1 = eql_loss(labels=label, logits=pred[1], weights=smpw)
    loss2 = eql_loss(labels=label, logits=pred[2], weights=smpw)
    classify_loss = (loss0+ loss1 + loss2)/3
