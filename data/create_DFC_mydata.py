...
def parse_args(argv):
    ...
    parser.add_argument('-i', '--input-path', default='../dfc_data/Track4', help='e.g. /path/to/DFC/Track4', required=False)
    parser.add_argument('-o', '--output-path', default='../dfc_data/block_pickle_w_mask0.1', help='e.g. /path/to/training_data_folder', required=False)
...
def create_train_val(argv=None):
    ...
    all_files = sorted(os.listdir(opts.input_path))
    train_data_psets = []
    train_label_psets = []
    val_data_psets = []
    val_label_psets = []
    ...
    for fname in all_files[:100]:
        pset = PointSet(os.path.join(opts.input_path, fname), os.path.join(opts.input_path + '-Truth', fname.replace('PC3', 'CLS')))
        [block_data_total, block_label_total] = pset.cloud2blocks_train(block_size=64,stride=32)
        print('block_data_total', block_data_total.__len__())
        print('block_label_total', block_label_total.__len__())
        
        if len(block_data_total)<16:
            print('file', fname)
#         pdb.set_trace()
        for block_data, block_label in zip(block_data_total, block_label_total):
            for sub_block_data, sub_block_label in zip(block_data, block_label):
                train_data_psets.append(sub_block_data)
                train_label_psets.append(sub_block_label)
                print('sub_block_data', sub_block_data.shape)
#         train_data_psets.append(np.concatenate(block_data_total, axis=0))
#         train_label_psets.append(np.concatenate(block_label_total, axis=0))
    ...
    for fname in all_files[100:]:
        ...
        for block_data, block_label in zip(block_data_total, block_label_total):
            for sub_block_data, sub_block_label in zip(block_data, block_label):
                val_data_psets.append(sub_block_data)
                val_label_psets.append(sub_block_label)
                print('sub_block_data', sub_block_data.shape)
...
def create_train_val_merge(argv=None):
    ...
    for fname in all_files:
        pset = PointSet(os.path.join(opts.input_path, fname), os.path.join(opts.input_path + '-Truth', fname))
        [block_data_total, block_label_total] = pset.cloud2blocks_train(block_size=64,stride=32)
        print('block_data_total', block_data_total.__len__())
        print('block_label_total', block_label_total.__len__())
