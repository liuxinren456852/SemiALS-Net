...
class PointSet(object):
    def __init__(self, points_file, class_file=''):
        ...
        ####create mask index]
        self.mask_boolen = np.zeros(len(self.x), dtype=bool)
        cx = (np.min(self.x) + np.max(self.x))/2.0
        cy = (np.min(self.y) + np.max(self.y))/2.0
#         1%, 10%, 20%, 30%, 50%
#         [4, 24, 44, 62, 88]
        ws = 4
        mask_idx = (self.x < cx + ws) & (self.x > cx - ws) & (self.x < cy + ws) & (self.y > cy -ws)
        
        self.mask_boolen[mask_idx] = True
    ...
    def cloud2blocks_test(self,block_size=100,stride=100):
        ...
        for idx in range(len(xbeg_list)): # idx=0...15
            ...
            block_label=np.stack([self.c[cond],self.mask_boolen[cond]], axis=1)
    ...
    def cloud2blocks(self,block_size=100,stride=100):
        ...
        for idx in range(len(xbeg_list)): # idx=0...15
            ...
            if idx==3 or idx==7 or idx==11 or idx==15:
                xcond=(self.x<=xbeg+block_size) & (self.x>=xbeg)
                ycond=(self.y<=ymax) & (self.y>=ybeg) #右边界
            else:
                xcond=(self.x<=xbeg+block_size) & (self.x>=xbeg)
                ycond=(self.y<=ybeg+block_size) & (self.y>=ybeg)
            cond=xcond&ycond
...
#             print('number of labeled points in block', np.sum(block_label[:,1]))
#             if np.sum(block_label[:,1])>1: #existing labeled points 
            block_data_list.append(block_data)
            block_label_list.append(block_label)
...
#                 print('number of labeled points in subblock', np.sum(block_label[:,1]))
#                 if np.sum(block_label[:,1])>1: #existing labeled points 
                block_data_one.append(block_data)
                block_label_one.append(block_label)
            block_data_total.append(block_data_one)  # maybe concat
