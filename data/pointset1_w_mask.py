...
class PointSet(object):
    def __init__(self, points_file, class_file=''):
        ...
        if points_file.endswith('.las') or points_file.endswith('.laz'):
            ...
            data = np.loadtxt(points_file,delimiter=delimiter,dtype='f8')
            self.x = data[:,0]
            self.y = data[:,1]
            self.z = data[:,2]
            self.i = data[:,3]
        ...
        mask_idx = np.random.choice(len(self.x), int(0.05*len(self.x)), replace=False)
